...
def pub_and_sub_calc(supply_voltage, last_loads, sub_loads, pub_voltages):
    ...
    feeder_voltage = [supply_voltage * 0.999 - max((fl - 1000) / 100, 0) for fl in last_loads]
...
def run_p1u_federate(fed_name, broker_address, feeders):
    ...
    for feeder in feeders:
        ...
        pub_voltages.append(h.helicsFederateRegisterGlobalTypePublication(vfed, pub_name, "double", "pu"))
        print(f"{fed_name}: publication {pub_name} registered")

    # Register subscriptions
    # subscribe to voltage supplied
    sub_voltage = h.helicsFederateRegisterSubscription(vfed, "full_network.voltage", "pu")
    ...
    while desiredtime <= end_time:
        ...
        if currenttime >= desiredtime:
            ...
            while iteration_state == h.helics_iteration_result_iterating:
                ...
                new_loads = pub_and_sub_calc(supply_voltage, last_loads, sub_loads, pub_voltages)
...
if __name__ == "__main__":
    fed_name = sys.argv[1] if len(sys.argv) >= 2 else "p1u"
